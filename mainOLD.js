...
const numberOfSeparationIterations = 2;
const STIFFNESS = 4.0;
...
function isBorderCell(index) {
    ...
    return x === 0 || x === X_CELLS - 1 || y === 0 || y === Y_CELLS - 1;
}


const SPATIAL_CELL_SPACING = 2.2 * particleRadius;  // Spacing of the spatial grid cells for particle separation
...
let cellParticleIndices = new Int32Array(particleCount);
...
function transferVelocitiesToParticles() {
    for (let component of ['x', 'y']) {
        ...
        for (let particle of particles) {
            ...
            // FIX ME
            var offset = component === 'x' ? 1 : X_CELLS;
            var valid0 = cellType[i1] === FLUID_CELL || cellType[i1 - offset] === FLUID_CELL ? 1.0 : 0.0;
            var valid1 = cellType[i2] === FLUID_CELL || cellType[i2 - offset] === FLUID_CELL ? 1.0 : 0.0;
            var valid2 = cellType[i3] === FLUID_CELL || cellType[i3 - offset] === FLUID_CELL ? 1.0 : 0.0;
            var valid3 = cellType[i4] === FLUID_CELL || cellType[i4 - offset] === FLUID_CELL ? 1.0 : 0.0;
...
function handleWallCollisions() {
    for (let particle of particles) {
        ...
        let minX = particleRadius + CELL_SPACING;
        let maxX = CANVAS_WIDTH - particleRadius - CELL_SPACING;
        let minY = particleRadius + CELL_SPACING;
        let maxY = CANVAS_HEIGHT - particleRadius - CELL_SPACING;
...
function createParticles() {
    const spacing = 2 * particleRadius * 1;
    const particlesPerRow = Math.floor(0.6 * CANVAS_WIDTH / spacing);
    const rows = Math.ceil(particleCount / particlesPerRow);
    ...
    for (let row = 0; row < rows; row++) {
        ...
        for (let col = 0; col < particlesPerRow; col++) {
            ...
            const x = col * spacing + particleRadius + CELL_SPACING + xOffset;
            const y = row * spacing + particleRadius + CELL_SPACING;
            const vx = 0.0;
            const vy = 0.0;
...
function particlesToBuffer() {
    let bufferData = new Float32Array(particleCount * 6);
...
gl.uniform1f(uPointSizeLocation, 2 * particleRadius);
...
function animate() {
    ...
    separateParticles(numberOfSeparationIterations);
